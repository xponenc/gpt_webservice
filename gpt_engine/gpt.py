...
async def generate_answer(query,  # запрос пользователя
                          ...
                          model='gpt-4o-mini',
                          temp=0):
    system_prompt = """Ты специалист в области страхования. Ответь на вопрос пользователя только на основании
     предоставленных тебе отрывков руководящего документа. Если в отрывках нет ответа на вопрос, то ответь,
      что ты не знаешь ответа. В ответе не упоминай документ. Пользователь вообще не должен знать о документе.
    Отрывки документа: 
    """
